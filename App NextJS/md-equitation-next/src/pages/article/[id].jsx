    ...
    return (
      <div ref={this.contextRef}>
        ...
        <Container className="mt-3" text>
          <Header as="h2">
            {article.map((article) => article.Name)}
            <Header.Subheader>
              {article.map((article) => article.Brand)}
            </Header.Subheader>
          </Header>
          <Image
            rounded
            src={article.map((article) => article.url)}
            size="medium"
            label={{
              color: 'brown',
          ...
          >
            <Header as="h2" color="brown">
              {article.map((article) => article.Price)}€
            </Header>
            <Divider />
          </Segment>
          ...
          <Segment basic floated="right" style={{ minWidth: '200px' }}>
            ...
            <ul>
              <li key={article.map((article) => article.articleid)}>
                {article.map((article) => article.Description)}
              </li>
              <li key={article.map((article) => article.articleid)}>
                Poids : {article.map((article) => article.Weight)} kg
              </li>
              <li key={article.map((article) => article.articleid)}>
                Hauteur : {article.map((article) => article.Height)} cm
              </li>
              <li key={article.map((article) => article.articleid)}>
                Largeur : {article.map((article) => article.Width)} cm
              </li>
              <li key={article.map((article) => article.articleid)}>
                Profondeur : {article.map((article) => article.Lenght)} cm
              </li>
            </ul>
            <Button
              circular
